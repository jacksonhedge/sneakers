...
- [x] **Aggregated balance endpoint + dashboard card** — `GET /api/balance` fans out to per-venue adapters concurrently and returns `{ totalCents, currency, byVenue }`. Dashboard balance card mounted above WalletStatusCard, self-hides when zero venues connected. Polymarket is the only live adapter; Kalshi registered, gated on credentials.
- [x] **Multi-venue credentials + connections schema** — migrations `034_credentials_multi_venue.sql` (drops polymarket-only check, adds `scope` read/trade), `035_user_venue_connections.sql` (Supabase-backed connections with `source` + `affiliate_clicked_at`, RLS-scoped to user), `036_credentials_kalshi_shape.sql` (nullable `api_secret_encrypted` for Kalshi/Opinion). `lib/connections.ts` rewritten on top; one-shot `migrateLocalConnections()` ports pre-Supabase localStorage entries on first mount.
- [x] **Kalshi balance adapter + venue-aware credentials route** — `lib/autotrade/kalshi.ts` (RSA-PSS signing, `fetchBalance`, `testConnection`), `lib/balance/venues/kalshi.ts` wrapper registered in `lib/balance/adapters.ts`. `/api/autotrade/credentials` now accepts `{ venue: 'kalshi', apiKey, privateKey }` and a `scope` field; Polymarket calls unchanged when `venue` omitted.
- [x] **Credential wizard modal** — `apps/platform/src/app/dashboard/credentials-wizard.tsx`, launched from the connections grid for credentialed venues. Venue-specific paste forms, scope toggle, affiliate-signup nudge, test-connection feedback.
