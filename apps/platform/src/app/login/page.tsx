...
function PositionBlock({ position, boost }: { position: number; boost: number }) {
  return (
    <div>
      <div className="text-xs text-blue-700 tracking-wider mb-1">{'>'} YOUR POSITION</div>
      <div className="text-5xl font-bold text-blue-600">#{position.toLocaleString()}</div>
...
export default async function LoginPage({
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center p-4 sm:p-8 overflow-hidden isolate bg-stone-50">
      ...
      />
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -z-10 w-[720px] h-[720px] rounded-full bg-blue-300/20 blur-[120px] pointer-events-none"
        aria-hidden
      />

      <div className="max-w-md w-full space-y-6 text-stone-900">
        <div className="text-center">
          <div className="text-xs text-blue-700 mb-4 tracking-wider font-semibold">
        ...
        {state.kind === 'no_email' && (
          <Card>
            ...
            <div className="text-xs text-stone-700">
              Sign in with your email and password. Forgot it? Reset your password with the
              link below the form.
            </div>
            <LoginForm />
        ...
        {state.kind === 'waitlist' && (() => {
          ...
          return (
            <Card>
              ...
              <div className="rounded-xl border border-blue-500/30 bg-blue-50 p-4 space-y-3">
                <div className="text-xs text-blue-700 tracking-wider font-semibold">
                  {'>'} UNLOCK ACCESS
                </div>
                <div className="flex items-start gap-3">
                  <span
                    className={`text-lg leading-none ${refs >= 1 ? 'text-blue-400' : 'text-stone-400'}`}
                  >
                    {refs >= 1 ? '✓' : '○'}
                  </span>
                ...
                <div className="text-[11px] text-stone-600 pt-2 border-t border-blue-500/20">
                  You have{' '}
                  <span className="text-blue-700 font-bold text-sm">{refs}</span>{' '}
        ...
        {state.kind === 'not_found' && (
          <Card>
            ...
            <Link
              href="/"
              className="block w-full text-center rounded-full bg-blue-600 text-white font-semibold px-6 py-3 ring-1 ring-blue-400 hover:bg-blue-400 transition"
            >
              JOIN THE WAITLIST →
            </Link>
