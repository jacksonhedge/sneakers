    ...
    <form onSubmit={submit} className="space-y-3">
      <div>
        ...
        <input
          ...
          className="w-full bg-black/40 backdrop-blur-sm border border-white/30 text-white px-4 py-3 focus:outline-none focus:border-blue-400 focus:bg-black/60 placeholder:text-white/30 tracking-[0.3em] font-semibold transition"
        />
      </div>
      ...
      <button
        ...
        className="w-full border border-blue-400 bg-blue-600 text-white font-semibold px-6 py-3 hover:bg-blue-400 hover:border-blue-300 transition disabled:opacity-50 tracking-wider"
      >
        {status === 'loading'
          ? hasCode
            ? 'SIGNING IN...'
...
function WaitlistSuccessCard({
  ...
  return (
    <div className="border border-blue-400/60 bg-black/60 backdrop-blur-sm p-5 text-white space-y-4">
      <div>
        <div className="text-sm text-blue-300">{'>'} You&apos;re on the list.</div>
        {typeof position === 'number' && (
          <div className="text-xs text-white/70 mt-1">
            ...
            {referrerCode && (
              <>
                {' '}Operator{' '}
                <span className="text-blue-400 font-semibold">{referrerCode}</span> just moved
                up.
              </>
            )}
          </div>
            ...
            return (
              <div
                ...
                className={`h-3 flex-1 rounded-full ${
                  filled ? 'bg-blue-500/30' : 'bg-blue-500 ring-1 ring-blue-300/40'
                }`}
      ...
      {link && (
        <div className="border-t border-blue-400/20 pt-4">
          <div className="text-[11px] tracking-wider text-blue-300/80 mb-2">YOUR LINK</div>
