  ...
  if (phase === 'saved') {
    return (
      <div className="space-y-3">
        <div className="rounded-lg border border-blue-400 bg-blue-50 px-4 py-4 text-blue-800">
    ...
    <form onSubmit={submit} className="space-y-3">
      <div className="relative">
        <input
          ...
          className="w-full bg-stone-50 border border-stone-300 text-stone-900 px-4 py-3 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-400/40 placeholder:text-stone-400 transition disabled:opacity-60"
        />
        <button
          type="button"
          onClick={() => setShowPw((s) => !s)}
          disabled={busy}
          className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] tracking-wider text-blue-700 hover:text-blue-800 font-semibold disabled:opacity-50"
        >
          {showPw ? 'HIDE' : 'SHOW'}
        </button>
      </div>
      <input
        ...
        className={`w-full bg-stone-50 text-stone-900 px-4 py-3 rounded-lg focus:outline-none focus:ring-1 placeholder:text-stone-400 transition disabled:opacity-60 ${
          confirm.length > 0 && !matches
            ...
            : 'border border-stone-300 focus:border-blue-500 focus:ring-blue-400/40'
        }`}
      />
      ...
      <ul className="text-[11px] text-stone-600 space-y-0.5 px-1">
        <li className={longEnough ? 'text-blue-700' : 'text-stone-500'}>
